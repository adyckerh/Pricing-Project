...
Solo travelers are often business travelers on a single budget; group bookings (couples, families) split cost across adults and tend to be leisure-oriented.
...
\textbf{Intercept.} Solo travelers have a much less-negative intercept ($-1.37$ vs.\ $-1.87$): substantially higher baseline booking propensity, consistent with business trips where dates are fixed.

\textbf{Price sensitivity.} Solo travelers are \textit{more} price-sensitive ($-1.44$ vs.\ $-1.28$). Solo travelers pay the full nightly rate themselves; group bookings split cost across adults, lowering per-person sensitivity.

\textbf{Brand and location.} Solo travelers weight brand ($+0.16$ vs.\ $+0.08$, roughly $2\times$) and location ($+0.08$ vs.\ $0.00$) much more heavily. This is the signature of business travel: loyalty programs matter and proximity to the destination is a hard constraint. Group travelers are essentially indifferent to location, consistent with leisure trips where the hotel itself is part of the experience.

\textbf{Review score and accessibility.} Group travelers lean more on external validation: review score ($+0.12$ vs.\ $+0.07$) and accessibility ($+0.05$ vs.\ $+0.02$). Families and couples reach consensus through social proof; solo travelers rely on brand signals they already trust.

\textbf{Promotion flag.} Both types respond positively, with group slightly more responsive ($+0.17$ vs.\ $+0.14$). Group trips are often discretionary, so a promotion can tip a leisure decision on the fence.

\textbf{Star rating.} Coefficients are close ($+0.43$ solo vs.\ $+0.39$ group), so both types value quality similarly once other features are controlled for.

\textbf{Contrast with Problem 4.} The early/late split produced a large price-sensitivity gap (0.63 units). The solo/group split produces a smaller price gap (0.16 units) but a much larger intercept gap (0.50 vs.\ 0.38 in early/late) and more meaningful differences in brand and location. Early/late separates customers on \textit{readiness to buy}; solo/group separates them on \textit{travel purpose} (business vs.\ leisure).
...
\bottomrule
\end{tabular}
\caption{Revenue gap from serving the compromise assortment $S$ instead of the type-specific optimum.}
\end{table}
...
\textbf{Contrast with Problem 5.} Problem 5's early/late split produced gaps up to \$0.92. This solo/group split produces a gap of \$3.36 on data2, more than $3\times$ larger. Trip-purpose signals (solo vs.\ group) drive real assortment-level lift; trip-timing signals like early/late produce smaller gaps.

\textbf{Main takeaway.} Solo vs.\ group disagrees on the assortment itself: solo wants fewer, higher-priced, brand-heavy hotels; group wants a broader assortment. That disagreement at the decision level is why personalization pays off here. Early/late and Saturday/non-Saturday mostly agree on the decision, so the personalization lift is modest.
...
Session top-1 accuracy asks whether the model picks the same hotel the real customer booked. The MNL outperforms the agent: 29.1\% vs.\ 19.6\% (487 vs.\ 327 correct out of 1,671 sessions). Exact-match top-1 is a strict bar: even the true model will miss most individual realizations because choices are noisy. The behavior comparison below is the primary evaluation, checking whether models reproduce the \textit{distribution} of booking behavior rather than each individual choice.

\subsection*{Behavior comparison}

Rather than asking whether the models predict the exact realized choice, we check whether they reproduce the \textit{aggregate behavior} in the data: booking rates across attribute bins and the average attribute profile of booked hotels.
